...
public enum FileTypeEnum {
    ...
    PROGRAME(FileCategoryEnum.OTHERS, 8, new String[]{".h", ".c", ".hpp", ".hxx", ".cpp", ".cc", ".c++", ".cxx", ".m", ".o", ".s", ".dll", ".cs", ".java", ".class", ".js", ".ts", ".css", ".scss", ".vue", ".jsx", ".sql", ".md", ".json", ".html", ".xml"}, "CODE"),
    ZIP(FileCategoryEnum.OTHERS, 9, new String[]{".rar", ".zip", ".7z", ".cab", ".arj", ".lzh", ".tar", ".gz", ".ace", ".uue", ".bz2", ".jar", ".iso", ".mpq"}, "压缩包"),
    ...
    public static FileTypeEnum getFileTypeBySuffix(String suffix) {
        for (int i = 0; i < FileTypeEnum.values().length; i++) {
            if (ArrayUtils.contains(FileTypeEnum.values()[i].getSuffixs(), suffix)) {
    ...
    public static FileTypeEnum getByType(Integer Type) {
        for (int i = 0; i < FileTypeEnum.values().length; i++) {
            if (FileTypeEnum.values()[i].getType().equals(Type)) {
                return FileTypeEnum.values()[i];
            }
        }
        return null;
